...
double LatticeBoltzmann::feq(double rho0,double Jx0,double Jy0,int i, double Ccelda){ //Se agrega Ccelda como argumento de la función
  if(i==0)
    return rho0*(1-(1-W0)*3*Ccelda*Ccelda); //AUX en forma explicita
  else
    return w[i]*(3*Ccelda*Ccelda*rho0+3*(V[0][i]*Jx0+V[1][i]*Jy0)); //Se colocan las constantes explicitamente, TresC2 y AUX0
}
void LatticeBoltzmann::Colisione(void){
  ...
  for(ix=0;ix<Lx;ix++)
    for(iy=0;iy<Ly;iy++){
      ...
      for(i=0;i<Q;i++)
        fnew[ix][iy][i]=UmUtau*f[ix][iy][i]+Utau*feq(rho0,Jx0,Jy0,i,Ccelda(ix,iy)); //Se agregua Ccelda como argumento
    }
}
...
void LatticeBoltzmann::Inicie(double rho0,double Jx0,double Jy0){
  for(int ix=0;ix<Lx;ix++)
    for(int iy=0;iy<Ly;iy++)
      for(int i=0;i<Q;i++)
        f[ix][iy][i]=feq(rho0,Jx0,Jy0,i,Ccelda(ix,iy)); //Se agregua Ccelda y se calcula para todas las casillas siguiendo el suavizado definida, tanh
}
void LatticeBoltzmann::ImponerCampos(int t){
  ...
  for(iy=0;iy<Ly;iy++){
   ...
   for(i=0;i<Q;i++){
     fnew[ix][iy][i]=feq(rho0,Jx0,Jy0,i,Ccelda(ix,iy));} //Se agregua Ccelda como argumento y se calcula para cada celda de la condición inicial
   }
}
...
double LatticeBoltzmann::Ccelda(int ix, int iy){
 int ix0=100;
 return 0.5/(0.5*tanh(ix-ix0)+1.5); //Suavizado realizado, antes de ix0 n=1, después n=2
}
