...
int main(void){
  ...
  double M,omega,T,x0,x1,V0,V1,l,x2,y2,Vx2, Vy2;
  double R01=5,R02=5;
  int i;
  double a=-M_PI/3;
  InicieAnimacion();
  ...
  Planeta[2].Inicie(x2, y2,0,Vx2, Vy2,   0, m2,  R02*10);
  std::ofstream fout("data.dat");
  for(l=tdibujo=0;l<10*T;l+=dt,tdibujo+=dt){
    ...
    /*  fout << l << " " << Planeta[0].Getx()*c-Planeta[0].Gety()*s<<" " << Planeta[0].Gety()*c+Planeta[0].Getx()*s<<" "<< Planeta[1].Getx()*c-Planeta[1].Gety()*s<<" " << Planeta[1].Gety()*c+Planeta[1].Getx()*s<<Planeta[2].Getx()*c+<<" "<< Planeta[2].Gety()<<" " << '\n';
     */
