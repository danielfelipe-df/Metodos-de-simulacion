...
void Colisionador::CalculeTodasLasFuerzas(Cuerpo * Molecula){
  ...
  for(i=0;i<Nx;i++)
    for(j=0;j<Ny;j++)
      { vector3D Punto1,Punto2,F;
	Punto1.cargue(0,120,0);Punto2.cargue(60,0,0);
	vector3D a,b;
	a=Punto1-Molecula[i*Ny+j].r;
	if(abs(a.x())-Molecula[i*Ny+j].R<=0)
	  {
	    
	    h=Molecula[i*Ny+j].R-abs(a.x());
	    F.cargue(k*pow(h,1.5),0,0);
	  }
	else if(abs(a.y())-Molecula[i*Ny+j].R<=0)
	  {
	   
	    h=Molecula[i*Ny+j].R-abs(a.y());
	    F.cargue(0,-k*pow(h,1.5),0);
	  }
	else
	  {
	    b=Punto2-Molecula[i*Ny+j].r;
	    if(abs(b.x())-Molecula[i*Ny+j].R<=0)
	  {
	    
	     h=Molecula[i*Ny+j].R-abs(b.x());
	    F.cargue(-k*pow(h,1.5),0,0);
	  }
	else if(abs(b.y())-Molecula[i*Ny+j].R<=0)
	  {
	   
	     h=Molecula[i*Ny+j].R-abs(b.y());
	    F.cargue(0,k*pow(h,1.5),0);
	  }
	else
	  {
	...
	Molecula[i*Ny+j].AgregueFuerza(F);
      }
}
//------------------ Funciones Globales -----------------
