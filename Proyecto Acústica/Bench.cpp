...
void LatticeBoltzmann::Colisione(void){
  ...
  for(ix=0;ix<Lx;ix++)
	...
	else if(ix==0){fnew[ix][iy][iz][2]=f[ix][iy][iz][1]; fnew[ix][iy][iz][1]=f[ix][iy][iz][2];}
...
void LatticeBoltzmann::ImponerCampos(int t){
  ...
  rho0=1*sin(omega*C*t); Jx0=Jx(ix,iy,iz,false); Jy0=Jy(ix,iy,iz,false); Jz0=Jz(ix,iy,iz,false);
  for(i=0;i<Q;i++)
...
void LatticeBoltzmann::Imprimase(const char * NombreArchivo){
  ...
  for(int ix=0;ix<Lx;ix++){
    for(int iy=0;iy<Ly;iy++){
      //for(int iz=0;iz<Lz;iz++){
        int z=0;
        rho0=rho(ix,iy,z,false);  //Jx0=Jx(ix,iy,iz,false); Jy0=Jy(ix,iy,iz,false); Jz0=Jz(ix,iy,iz,false);
