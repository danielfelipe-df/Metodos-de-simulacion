...
public:
  ...
  void AgregueFuerzaPared(vector3D dM){M+=dM;};
  double MuestreMomentum(void){return M.x();};
...
public:
  ...
  void CalculeTodasLasFuerzas1(Cuerpo * Molecula);
};
...
void Colisionador::CalculeTodasLasFuerzas(Cuerpo * Molecula){
  ...
  vector3D S1, S2, S3, S4, F3, F4, P13, P24;
  double h, FP;
...
void Colisionador::CalculeTodasLasFuerzas1(Cuerpo * Molecula){
  int i,j;
  ...
  Molecula[i].AgregueFuerza(F3);
  Molecula[i].AgregueFuerzaPared(F4);
  }
...
int main(void){
  ...
  for(t=0;t<=teq;t+=dt){
	...
	VelocidadParticulaenteq[i]= Molecula[i].GetVx();
     ...
     for(t=teq;t<=(teq+T);t+=dt){
    for(i=0;i<N;i++) Molecula[i].Mueva_r(dt,Zi);
    Newton.CalculeTodasLasFuerzas1(Molecula);
    for(i=0;i<N;i++) Molecula[i].Mueva_V(dt,Coeficiente1);
    for(i=0;i<N;i++) Molecula[i].Mueva_r(dt,Xi);
    Newton.CalculeTodasLasFuerzas1(Molecula);
    for(i=0;i<N;i++) Molecula[i].Mueva_V(dt,Lambda);
    ...
    for(i=0;i<N;i++) Molecula[i].Mueva_r(dt,Zi);
     ...
     //_________CALCULA VELOCIDAD TOTAL EN TIEMPO DE EQUILIBRIO__________
  double Total;
  double Promedio;
  for (i=0; i<N; i++){
  ...
  double sigma, sigmateo;
  double dif2, suma2;
  double MomentumIndividual;
  ...
  for(i=0; i<N; i++){
    ...
    suma2 +=dif2;
    MomentumIndividual = Molecula[i].MuestreMomentum();
    MomentumTotal += MomentumIndividual;
  }

  Presion = MomentumTotal/(N*T*Lx);  
  sigmateo = pow(kT, 0.5);
  sigma = pow((suma2/N),0.5);

  suma2=0;
  cout<<sigma<<endl;
  //_____________IMPRIME ERROR DE SIGMA_________
cout<<"Error porcentual =  "<<abs((sigma-sigmateo)*100.0/sigmateo)<<endl;;



  //_____________IMPRIME KT VS PRESIÓN
  cout<<"KT experimental vs  Presion"<<endl;
  cout<<pow(sigma,2)<<'\t'<<Presion<<endl;  
